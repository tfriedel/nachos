...
Machine::ReadMem(int addr, int size, int *value)
{
    ...
    DEBUG(dbgMach, "Reading VA " << addr << ", size " << size);
	...
	*value = WordToHost(data);
    ...
    DEBUG(dbgMach, "\tvalue read = " << *value);
    return (TRUE);
}
...
Machine::WriteMem(int addr, int size, int value)
{
    ...
    DEBUG(dbgMach, "Writing VA " << addr << ", size " << size << ", value " << value);
...
Machine::Translate(int virtAddr, int* physAddr, int size, bool writing)
{
    ...
    DEBUG(dbgMach, "\tTranslate " << virtAddr << (writing ? " , write" : " , read"));

// check for alignment errors
    if (((size == 4) && (virtAddr & 0x3)) || ((size == 2) && (virtAddr & 0x1))){
	DEBUG(dbgMach, "Alignment problem at " << virtAddr << ", size " << size);
...
// from the virtual address
	...
	if (vpn >= pageTableSize) {
	    DEBUG(dbgMach, "Illegal virtual page # " << virtAddr);
	    return AddressErrorException;
	} else if (!pageTable[vpn].valid) {
	    DEBUG(dbgMach, "Invalid virtual page # " << virtAddr);
	    return PageFaultException;
	}
	entry = &pageTable[vpn];
    } else {
        for (entry = NULL, i = 0; i < kernel->TLBSize; i++)
    	    if (tlb[i].valid && (tlb[i].virtualPage == ((int)vpn))) {
		entry = &tlb[i];			// FOUND!
		break;
	    }
	if (entry == NULL) {				// not found
    	    DEBUG(dbgMach, "Invalid TLB entry for this virtual page!");
	...
	DEBUG(dbgMach, "Write to read-only page at " << virtAddr);
    ...
    if (pageFrame >= kernel->NumPhysPages) {
	DEBUG(dbgMach, "Illegal pageframe " << pageFrame);
    ...
    *physAddr = pageFrame * PageSize + offset;
    ...
    DEBUG(dbgMach, "phys addr = " << *physAddr);
    return NoException;
}
