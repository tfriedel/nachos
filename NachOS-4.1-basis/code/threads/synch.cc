...
void Semaphore::P()
{
  ...
  DEBUG(dbgSynch, kernel->currentThread->getName() << ": " << __FILE__ << ":" << __LINE__ << ": Semaphore->P() (name: "
       << name << ") called.\n");
  value--;     // consume its value
  if (value < 0)
  { // semaphore not available
    queue->Append(kernel->currentThread); // so go to sleep
    DEBUG(dbgSynch, kernel->currentThread->getName() << ": " << __FILE__ << ":" << __LINE__ << ": Semaphore->P() (name: " << name
...
void Semaphore::V()
{
  ...
  value++;
  DEBUG(dbgSynch, kernel->currentThread->getName() << ": " << __FILE__ << ":" << __LINE__ << ": Semaphore->V() (name: "
     << name << ") called.\n");
