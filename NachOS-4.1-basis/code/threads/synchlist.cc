...
SynchList<T>::SynchList()
{
    ...
    listEmpty = new Condition(lock,"list empty cond");
}
...
//----------------------------------------------------------------------
// SynchList<T>::GetFirst
//      Get the first "item" from the front of the list.
// 
// Returns:
//	 First item, NULL if nothing on the list.
//----------------------------------------------------------------------

template <class T>
T 
SynchList<T>::getFirst(bool block)
{
    lock->Acquire();            
    // If block is true, wait until list isn't empty
    if (block)          
        while (list->IsEmpty()) 
            listEmpty->Wait();  
    // get first element
    T item = list->getFirst();    
    ASSERT(!block || item != NULL);
    lock->Release();
    return item;
}
